...
def read_docs_to_data(path, files, file_unknown, language):
    ...
    punct_unknown_arr = np.array(punct_unknown)
    av_punct_known_arr = np.array(av_punct_known)
    line_endings_unknown_arr = np.array(line_endings_unknown)
    av_line_endings_known_arr = np.array(av_line_endings_known)
    linelength_unknown_arr = np.array(linelength_unknown)
    av_linelength_known_arr = np.array(av_linelength_known)

    punct_sim = metrics.pairwise.cosine_similarity(punct_unknown_arr.reshape(1, -1), av_punct_known_arr.reshape(1, -1))  # vectors of length 8
    line_endings_sim = metrics.pairwise.cosine_similarity(line_endings_unknown_arr.reshape(1, -1), av_line_endings_known_arr.reshape(1, -1))  # vectors of length 7
    linelength_sim = metrics.pairwise.cosine_similarity(linelength_unknown_arr.reshape(1, -1), av_linelength_known_arr.reshape(1, -1))  # vectors of length 3
    ...
    vec_sim_mean = np.mean(vec_sim)

    if args.combo >= 2:
        instance.append(float(vec_sim_mean))
...
def vector_similarity(doc1_tokens, doc2_tokens):
    ...
    return metrics.pairwise.cosine_similarity(v1_2d, v2_2d)
...
if __name__ == "__main__":
    ...
    if args.split:  # if splitting of 1 data set into train and test
        ...
        for train, test in skf.split(full,full_labels):
